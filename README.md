...
* After discarding the last 2 digits, the number become a 17-digit number with _a 9 as it's last digit_. And since the last digit is 9, we know the the last digit of square root of it must be 3 or 7, so I use _mod(sqrt(x), 10) == 3 || mod(sqrt(x),10) == 7_ to check the answer actually meets the topic.
### Checking
