...
%% Search the answer by swapping every _ from 0 to 9
...
% Only 8 _ since already know that the last _ is 0
for a1 = 0 : 9
    for a2 = 0 : 9
        for a3 = 0 : 9
            for a4 = 0 : 9
...
% Times every digit to the answer itself
for ii = 1 : 9
    z(ii) = y(ii) * p;
end
...
% multiplication, and some remain 9-digit, so I divide them into 2 groups
...
% We've known that the square of the answer is a 19-digit number.
for ii = 1 : 19
    answer(ii) = 0;
end
...
disp(['The square of the answer : ', num2str(answer)]);